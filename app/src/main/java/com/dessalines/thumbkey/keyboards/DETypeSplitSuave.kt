...
private val GRID_1_2 = SuaveKeyDef(" ", top = "up", bottom = "down", left = "left", right = "right", bg = SURFACE_VARIANT, swipeType = FOUR_WAY_CROSS, slide = SlideType.MOVE_CURSOR)
...
private val GRID_2_2 = SuaveKeyDef("shift", top = "-", left = "—", topLeft = ";", topRight = ":", right = "_", bottom = "^", bottomLeft = "%", bottomRight = "&", bg = SURFACE_VARIANT)
    ...
    val result = grid.map { row ->
        row.map { item ->
            when (item) {
                is KeyItemC -> item
                is SuaveKeyDef -> {
                    val backgroundColor = item.bg ?: if (mode == SuaveMode.CTRLED || mode == SuaveMode.ALTED) SURFACE_VARIANT else SURFACE
                    ...
                    fun process(text: String?, isCenter: Boolean = false): KeyC? {
                        ...
                        when (text.lowercase()) {
                            "shift" -> return if (mode == SuaveMode.SHIFTED) {
                                KeyC(display = KeyDisplay.IconDisplay(Icons.Outlined.KeyboardArrowUp), capsModeDisplay = KeyDisplay.IconDisplay(Icons.Outlined.KeyboardCapslock), action = ToggleShiftMode(false), size = LARGE, color = SECONDARY)
                            } else {
                                KeyC(display = KeyDisplay.IconDisplay(Icons.Outlined.ArrowDropUp), action = ToggleShiftMode(true), size = LARGE, color = SECONDARY)
                            }
                            "ctrl" -> {
                                val isActive = (mode == SuaveMode.CTRLED)
                                return KeyC(display = KeyDisplay.TextDisplay("ctrl"), action = ToggleCtrlMode(!isActive), color = if (isActive) PRIMARY else SECONDARY)
                            }
                            "alt" -> {
                                val isActive = (mode == SuaveMode.ALTED)
                                return KeyC(display = KeyDisplay.TextDisplay("alt"), action = ToggleAltMode(!isActive), color = if (isActive) PRIMARY else MUTED)
                            }
                            "esc" -> return KeyC(display = KeyDisplay.TextDisplay("esc"), action = SendEvent(KeyEvent(KeyEvent.ACTION_DOWN, KeyEvent.KEYCODE_ESCAPE)), color = MUTED)
                            "return" -> return KeyC(display = KeyDisplay.IconDisplay(Icons.AutoMirrored.Outlined.KeyboardReturn), action = IMECompleteAction, size = LARGE, color = SECONDARY)
                            "tab" -> return KeyC(display = KeyDisplay.IconDisplay(Icons.AutoMirrored.Outlined.KeyboardTab), action = CommitText("\t"), color = SECONDARY)
                            "enter" -> return KeyC(display = null, action = CommitText("\n"))
                            "backspace" -> return KeyC(display = KeyDisplay.IconDisplay(Icons.AutoMirrored.Outlined.KeyboardBackspace), action = DeleteKeyAction, size = LARGE, color = SECONDARY)
                            "left" -> return KeyC(display = KeyDisplay.TextDisplay("←"), action = SendEvent(KeyEvent(KeyEvent.ACTION_DOWN, KeyEvent.KEYCODE_DPAD_LEFT)), color = MUTED)
                            ...
                            "selectall" -> return KeyC(display = KeyDisplay.IconDisplay(Icons.Outlined.SelectAll), action = SelectAll, swipeReturnAction = SelectLineWithCursor, color = MUTED)
                            "cut" -> return KeyC(display = KeyDisplay.IconDisplay(Icons.Outlined.ContentCut), action = Cut, color = MUTED)
                            "undo" -> return KeyC(display = KeyDisplay.IconDisplay(Icons.AutoMirrored.Outlined.Undo), action = Undo, color = MUTED)
                            "redo" -> return KeyC(display = KeyDisplay.IconDisplay(Icons.AutoMirrored.Outlined.Redo), action = Redo, color = MUTED)
                            "paste" -> return KeyC(display = KeyDisplay.IconDisplay(Icons.Outlined.ContentPaste), action = Paste, swipeReturnAction = ToggleClipboardMode(true), color = MUTED)
                            "settings" -> return KeyC(display = KeyDisplay.IconDisplay(Icons.Outlined.Settings), action = GotoSettings, color = MUTED)
                            "hide" -> return KeyC(display = KeyDisplay.IconDisplay(Icons.Outlined.HideImage), action = ToggleHideLetters, color = MUTED)
                            "ime" -> return KeyC(display = KeyDisplay.IconDisplay(Icons.Outlined.Keyboard), action = SwitchIME, color = MUTED)
                            "voice" -> return KeyC(display = KeyDisplay.IconDisplay(Icons.Outlined.Mic), action = SwitchIMEVoice, color = MUTED)
                            "lang" -> return KeyC(display = KeyDisplay.IconDisplay(Icons.Outlined.Language), action = SwitchLanguage, color = MUTED)
                            "move" -> return KeyC(display = KeyDisplay.IconDisplay(Icons.Outlined.LinearScale), action = MoveKeyboard.CycleRight, color = MUTED)
                        }
                    ...
                    KeyItemC(
                        ...
                        longPress = if (item.center == "shift") ToggleCapsLock else if (item.center == "return") CommitText("\n") else if (item.center == "backspace") DeleteWordBeforeCursor else null,
                        nextTapActions = if (item.center == " ") listOf(ReplaceLastText(", ", trimCount = 1), ReplaceLastText(". "), ReplaceLastText("? "), ReplaceLastText("! "), ReplaceLastText(": "), ReplaceLastText("; ")) else null,
